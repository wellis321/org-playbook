<?php
$pageTitle = 'Development Roadmap | Organisation Playbook';
$pageDescription = 'Our plan for developing the Organisation Playbook into a comprehensive resource.';
    ...
    <div class="container mx-auto max-w-5xl px-4">
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-1 gap-6">
                <?php foreach ($roadmapSections as $section): ?>
                    <div class="rounded-xl bg-white p-6 shadow-sm">
                        <div class="mb-2 flex items-center justify-between">
                            <h3 class="text-xl font-semibold text-slate-800"><?php echo htmlspecialchars($section['title']); ?></h3>
                            <span class="rounded-full px-3 py-1 text-sm <?php
                                echo $section['status'] === 'In Progress' ? 'bg-blue-100 text-blue-800' :
                                    ($section['status'] === 'Planning' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-800');
                            ?>">
                                <?php echo htmlspecialchars($section['status']); ?>
                            </span>
                        </div>
                        <p class="mb-4 text-slate-600"><?php echo htmlspecialchars($section['description']); ?></p>
                        <div class="h-2.5 w-full rounded-full bg-slate-200">
                            <div
                                class="h-2.5 rounded-full bg-blue-600"
                                style="width: <?php echo $section['percentComplete']; ?>%"
                            ></div>
                        </div>
                        <div class="mt-2 text-right text-sm text-slate-500">
                            <?php echo $section['percentComplete']; ?>% complete
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
